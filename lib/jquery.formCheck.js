		...
		return this.each(function(index, el) {
			...
			var arrMsg = {
				...
				phoneCheck: {
					...
					phonemsgL: '请输入正确长度的电话号码'
				},
				IDCheck: {
					IDPassMsg: 'isPass',
					IDMsg: '请输入正确的身份证号码',
					IDEmptyMsg: '请输入身份证号码',
					IDmsgL: '请输入正确长度的身份证号码'
				},
				emailCheck: {
					emailPassMsg: 'isPass',
					emailMsg: '请输入正确格式的邮箱',
					emailEmptyMsg: '请输入邮箱',
					emailmsgL: '请输入正确长度的邮箱'
				},
				userNameCheck: {
					userNamePassMsg: 'isPass',
					userNameMsg: '请输入正确格式的用户名',
					userNameEmptyMsg: '请输入用户名',
					userNamemsgL: '请输入正确长度的用户名'
				},
				notEmptyCheck: {
			...
			function showResult() {
				...
				var $info=' \n <div class="u-info">'
                    $info+='   <i class="u-icon u-icon-dui "></i>'
                     $info+=   ' <div class="u-info-tips">'
                      $info+=   '  <p class="u-wrong v-wrong">'
                       $info+=     $(This).attr('data-xg-msg');
                     $info+=     ' </p>'
                     $info+=  '  </div>'
                    $info+=  ' </div>'

				if($(This).next().hasClass('u-info')){
					$(This).next('.u-info').find('.u-info-tips .v-wrong').text($(This).attr('data-xg-msg'));
				}else{
					$(This).after($info);
				}
				if (resultBool) {
					$(This).next('.u-info').find('.u-icon-dui').removeClass('z-act');
					$(This).removeClass('z-act')
					$(This).next('.u-info').on('mouseover',function(){

	                 $(This).next('.u-info').find('.u-info-tips').hide();
	                })
				} else {
					$(This).next('.u-info').find('.u-icon-dui').addClass('z-act');
					$(This).addClass('z-act')
					$(This).next('.u-info').on('mouseover',function(){

	                 $(This).next('.u-info').find('.u-info-tips').show();
	                })
	                $(This).next('.u-info').on('mouseout',function(){
			 ...
			 *姓名的验证长度
			*/
			function forLength(s,nMin, nMax) {
				s += '';
				var ref= '/^.{' + nMin + ',' + nMax + '}$/'
				if(s.match(eval(ref))){
					return true;
					...
					if (options.msgL) {
						options.msg = options.msgL;
						obj.msg= options.msgL;
					} else {
						options.msg = arrMsg.nameCheck.namemsgL;
						obj.msg= arrMsg.nameCheck.namemsgL;
					}
					return false;
				}
			}
			...
			function chnName(s, nMin, nMax) {
				...
				if (s) {
					isEmpty = false;
					return s.match(eval(ref)) ? forLength(s,nMin, nMax) : false;
				} else {
					isEmpty = true;
					return false;
				}
